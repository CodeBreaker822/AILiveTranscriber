...
{
    public function connectivity(HostedTranscriptionApiService $api): JsonResponse
    {
        return response()->json(['online' => $api->serverIsReachable()]);
    }
    ...
    public function download(AppUpdateService $updates): StreamedResponse|JsonResponse
    {
        try {
            $remote = $updates->download();
        } catch (SpeechToTextException $exception) {
