...
class SpeechAudioFilterService
{
    public function __construct(
        private readonly SileroVadService $vad,
    ) {
    }
    ...
    {
        $vad = $this->vad->detect((string) $audio['path']);
    ...
    private function mergeSpeechSegments(array $audio, array $segments): array
    {
        ...
        return [
            ...
            'duration_ms' => $this->probeDurationMs($outputPath),
        ];
    }
    ...
    private function probeDurationMs(string $path): int
    {
        ...
        return max(1, (int) round(((float) trim($process->getOutput())) * 1000));
    }
