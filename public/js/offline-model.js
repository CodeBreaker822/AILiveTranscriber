(() => {
    ...
    ready(() => {
        ...
        if (!modelList) {
            ...
            modelList.className = 'mt-3 grid min-h-0 gap-1.5 overflow-y-auto pr-1 sm:grid-cols-2';
        ...
        const installedModels = (status) => (Array.isArray(status?.models) ? status.models : [])
            .filter((model) => model?.installed === true && model?.supported !== false);
        ...
        const renderModels = () => {
            ...
            models.forEach((model) => {
                ...
                option.className = `flex min-w-0 items-center justify-between gap-2 rounded-lg border px-2.5 py-2 ${
                    selected
                ...
                option.innerHTML = `
                    <span class="min-w-0">
                        <span class="block truncate text-xs font-semibold text-white"></span>
                        <span class="mt-0.5 block text-[0.65rem] text-slate-400"></span>
                    </span>
                    <button type="button" class="shrink-0 cursor-pointer rounded-md px-2 py-1 text-[0.65rem] font-semibold transition disabled:cursor-default disabled:opacity-70 ${
                        installed ? 'bg-emerald-300/15 text-emerald-200' : 'bg-cyan-300 text-slate-950'
                    }"></button>
                `;
                option.querySelector('span span:first-child').textContent = String(model.label || model.id || 'Whisper');
        ...
        const showCatalog = () => {
            ...
            message.textContent = 'Choose a Whisper model. Models are downloaded after installation and are not included in the app installer.';
            compactLabel.textContent = 'Offline model download';
        ...
        const handleDownloadEvent = async (event) => {
            ...
            if (event.type === 'source') {
                message.textContent = `Downloading ${activeModel} from ${String(event.host || 'the model server')}.`;
                return;
            }
        ...
        async function startDownload(model) {
            ...
            try {
                ...
                await refreshStatus();
                title.textContent = 'Offline model installed';
                message.textContent = 'Offline transcription is ready. You can choose the installed model from the transcription controls.';
