...
class OfflineWhisperConfigurationTest extends TestCase
{
    ...
    public function test_desktop_profiles_resources_once_and_passes_a_bounded_thread_budget(): void
    {
        ...
        $cargo = file_get_contents($root.'/src-tauri/Cargo.toml');
        $this->assertStringContainsString('cuda = ["whisper-rs/cuda"]', $cargo);
        $this->assertStringContainsString('offline-whisper-worker', $cargo);
        $this->assertStringContainsString('nvidia-smi', $main);
        $this->assertStringContainsString('detect_cuda_gpu()', $main);
        $this->assertStringContainsString('detect_vulkan_gpu()', $main);
        $this->assertStringContainsString('windows_dll_available("cudart64")', $main);
        $this->assertStringContainsString('windows_dll_available("cublasLt64")', $main);
        $this->assertStringContainsString('windows_dll_available("vulkan-1")', $main);
        $this->assertStringContainsString('start_external_offline_whisper_worker', $main);
