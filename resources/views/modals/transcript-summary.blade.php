<dialog data-summary-dialog class="fixed inset-0 z-[80] m-auto hidden max-h-[calc(100dvh-2rem)] w-[min(94vw,46rem)] overflow-hidden rounded-lg border border-white/10 bg-slate-950 p-0 text-slate-100 shadow-2xl backdrop:bg-slate-950/85">
    <section class="flex max-h-[calc(100dvh-2rem)] min-h-0 flex-col p-4 sm:p-5" role="document">
        ...
        <pre data-summary-text class="mt-3 min-h-40 flex-1 overflow-y-auto whitespace-pre-wrap break-words rounded-lg border border-white/10 bg-slate-900/80 p-4 font-sans text-sm leading-6 text-slate-200">No summary has been created for this project.</pre>
        <p data-summary-error class="mt-2 hidden shrink-0 text-sm text-rose-300"></p>

        <footer class="mt-3 flex shrink-0 items-center justify-between gap-3">
